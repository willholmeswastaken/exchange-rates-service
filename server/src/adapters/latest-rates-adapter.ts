...
const getLatestRates = async (currency: string): Promise<IExchangeRate[]> => {
  const response: ILatestResponse = await (await axios.get(`${config.EXCHANGE_RATES_BASE_URL}?base=${currency.toUpperCase()}`)).data;
